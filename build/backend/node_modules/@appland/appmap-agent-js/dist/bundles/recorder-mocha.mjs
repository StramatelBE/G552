...
import { hooks as hooks$1 } from '../../lib/node/mocha-hook.mjs';
...
const {
  Number: { isNaN: isNaN$1, parseInt: parseInt$3 },
  Math: { max },
} = globalThis;
...
const matchVersion = (actual, target) => {
  ...
  for (let index = 0; index < length; index += 1) {
    ...
    const segment1 = parseInt$3(segments1[index], 10);
    assert(!isNaN$1(segment1), "could not parse version", AssertionError);
    const segment2 = parseInt$3(segments2[index], 10);
    assert(!isNaN$1(segment2), "could not parse version", AssertionError);
...
const { Map: Map$4, RegExp: RegExp$2 } = globalThis;

const regexps = new Map$4();

const makeRegExp = (source, flags) => {
  try {
    return new RegExp$2(source, flags);
  } catch (error) {
    logError(
...
const normalizeScenarios = (scenarios, base) =>
  toEntries$4(scenarios).map(([key, value]) => ({
    base,
    key,
    value,
  }));
...
const {
  ...
  /* c8 ignore start */
  Object: {
    hasOwn: hasOwn$1 = (obj, key) => getOwnPropertyDescriptor$3(obj, key) !== undefined$9,
  },
  /* c8 ignore stop */
} = globalThis;
...
const {
  ...
  /* c8 ignore start */
  Object: {
    hasOwn = (obj, key) => getOwnPropertyDescriptor$2(obj, key) !== undefined$8,
  },
  /* c8 ignore stop */
} = globalThis;
...
const parsePosition = (string) => {
  ...
  return {
    line: parseInt$2(parts[1]),
    column: parseInt$2(parts[2]),
  };
};
...
const { Map: Map$3, String: String$4 } = globalThis;
...
const { String: String$3, parseInt: parseInt$1, undefined: undefined$7 } = globalThis;
...
const {
  Array: { isArray: isArray$2, from: toArray$1 },
  ...
  if (enabled.value) {
    ...
    buffer.push([
      ...
      isArray$2(parameters)
        ...
        : fromEntries$1(
            toArray$1(toEntries$1(parameters)).map(([name, parameter]) => [
              name,
              serialize(serialization, enabled, parameter),
            ]),
          ),
...
const {
  Set,
  Map,
  Array: { isArray: isArray$1, from: toArray },
  Object: toObject,
...
const { undefined: undefined$1, parseInt, Promise: Promise$1 } = globalThis;
...
const validateMocha = (Mocha) => {
  const prototype = coalesce(Mocha, "prototype", undefined$1);
  const version = coalesce(prototype, "version", undefined$1);
  assert(
    !logErrorWhen(
...
const recordAsync = (configuration) => {
  ...
  hooks$1.beforeEach = async (context) => {
